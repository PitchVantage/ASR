...
#Script: run_generalized_segments_TRAIN.sh
#Author: Multiple
#Last Updated: 05_05_16

#Purpose: Runs end-to-end train and test with segment files where only *TRAIN* is segmented. Segments files are specific to train and test and can ONLY list the files being used. You can't use a "master" segments file for both train and test.

#Command Line: ./run_generalized_segments_TRAIN.sh -p [# of processors] -n [path/to/training/.wav/files] -t [path/to/testing/.wav/files] -i [path/to/input/folder] -i [path/to/input/folder] -g [path/to/train/segments]

#Example Command Line: ./run_generalized_segments_TRAIN.sh -p 4 -n /Volumes/My_Passport_Mac/TED_Kaldi_Files/TED_wav/train/ -t /Volumes/My_Passport_Mac/TED_Kaldi_Files/TED_wav/test/ -i /Volumes/poo/ASR/egs/Input_Folders/input_V2/ -g /Volumes/poo/ASR/egs/Input_Folders/Segments_folder/segmentsTRAIN
...
# -g = full path to train segments file
...
waves_dir="waves_dir"
mkdir data
mkdir data/train_dir
mkdir data/test_dir

while getopts "p:n:t:a:s:i:g:z:" opt; do
    case $opt in
        p)
        ...
        g)
            # make symbolic links from locations of true data to directories expected by kaldi
            g=$OPTARG
            ln -s $g data/train_dir/segments
            ;;
        \?)
...
# -n and -t flags used (data already split)
    ...
    #added fourth parameter to include split amount for training
    # (used in create_waves_test_train.pl inside prepare data)
    local/prepare_data_generalized_segments_train.sh "" $train_dir $test_dir $sDefault || \
    printf "\n####\n#### ERROR: prepare_data.sh \n####\n\n";

else
#if -a flag used (data must be split)

    local/prepare_data_generalized_segments.sh $waves_dir $train_dir $test_dir $sDefault|| \
    printf "\n####\n#### ERROR: prepare_data.sh \n####\n\n";
