...
if [ $stage -le 1 ]; then
  ...
  ${PATH_TO_KALDI}/egs/nextiva_recipes/steps/analyze_lats.sh --cmd "$cmd" $iter_opt $graphdir $dir
fi

if ! $skip_scoring ; then
  [ ! -x ${PATH_TO_KALDI}/egs/nextiva_recipes/local/score.sh ] && \
    echo "Not scoring because local/score.sh does not exist or not executable." && exit 1;
  ${PATH_TO_KALDI}/egs/nextiva_recipes/local/score.sh --cmd "$cmd" $scoring_opts $data $graphdir $dir ||
