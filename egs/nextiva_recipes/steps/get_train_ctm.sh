...
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then
    $cmd JOB=1:$nj $dir/log/get_ctm.JOB.log \
      ...
      ${PATH_TO_KALDI}/egs/nextiva_recipes/utils/int2sym.pl -f 5 $lang/words.txt \| \
...
if [ $stage -le 1 ]; then
  if [ -f $data/segments ] && $use_segments; then
    ...
    for n in `seq $nj`; do gunzip -c $dir/ctm.$n.gz; done | \
      ${PATH_TO_KALDI}/egs/nextiva_recipes/utils/convert_ctm.pl $data/segments $data/reco2file_and_channel > $dir/ctm || exit 1;
  else
