...
nj=2
cmd=run.pl
mfcc_config=${PATH_TO_KALDI}/egs/nextiva_recipes/conf/mfcc.conf
compress=true
...
if [ -f ${PATH_TO_KALDI}/egs/nextiva_recipes/path.sh ]; then . ${PATH_TO_KALDI}/egs/nextiva_recipes/path.sh; fi
...
${PATH_TO_KALDI}/egs/nextiva_recipes/utils/validate_data_dir.sh --no-text --no-feats $data || exit 1;
...
for n in $(seq $nj); do
  ...
  ${PATH_TO_KALDI}/egs/nextiva_recipes/utils/create_data_link.pl $mfccdir/raw_mfcc_$name.$n.ark
done

if [ -f $data/segments ]; then
  echo "$0 [info]: segments file exists: using that."

# handling a segmented transcript
  split_segments=""
  for n in $(seq $nj); do
    split_segments="$split_segments $logdir/segments.$n"
  done

  ${PATH_TO_KALDI}/egs/nextiva_recipes/utils/split_scp.pl $data/segments $split_segments || exit 1;
  ...
  ${PATH_TO_KALDI}/egs/nextiva_recipes/utils/split_scp.pl $scp $split_scps || exit 1;
